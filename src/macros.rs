...
#[macro_use]
macro_rules! col_int_key { ($field:expr) => { col_def!($field,"INTEGER PRIMARY KEY ASC")  } }
#[macro_use]
...
macro_rules! insert_row {
    ($table_name:expr, $values:expr) => {
        format!("INSERT INTO {} VALUES ({}); ", &$table_name, $values)
    }
}
#[macro_export]
macro_rules! insert_row_cols {
    ($table_name:expr, $columns:expr, $values:expr) => {
        format!("INSERT INTO {} ({}) VALUES ({}); ", &$table_name, $columns, $values)
    }
}